...
class LRUDataloader():
    ...
    def get_pytorch_test_subset_dataloader(self):
        ...
        retrieved_file = pickle.load(open(os.path.join(retrieved_file_path, 'retrieved.pkl'), 'rb'))
    ...
    def _get_train_dataset(self):
        return LRUTrainDataset(self.args, self.train, self.max_len, self.sliding_size, self.rng)
    ...
    def _get_eval_dataset(self, mode):
        if mode == 'val':
            return LRUValidDataset(self.args, self.train, self.val, self.max_len, self.rng)
        elif mode == 'test':
            return LRUTestDataset(self.args, self.train, self.val, self.test, self.max_len, self.rng)
    ...
    def __init__(self, args, u2seq, max_len, sliding_size, rng):
        ...
        for u in sorted(u2seq.keys()):
            ...
            if len(seq) < self.max_len + self.sliding_step:
                ...
                self.all_seqs.extend([seq[i:i + max_len] for i in start_idx])
