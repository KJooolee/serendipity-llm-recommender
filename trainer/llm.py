...
def llama_collate_fn_w_truncation(llm_max_length, eval=False):
    def llama_collate_fn(
        ...
        for i in range(len(batch)):
            ...
            item_candidates = batch[i]['candidates']                # projector용
            category_candidates = batch[i]['category_candidates']  # projector용
            ...
            # === embedding 생성 + projector inject 추가 ===
            input_ids_tensor = torch.tensor(input_ids).unsqueeze(0).to(device)
            inputs_embeds = tokenizer.model.embed_tokens(input_ids_tensor).squeeze(0)

            item_token_id = tokenizer.convert_tokens_to_ids("[ItemEmb]")
            cat_token_id = tokenizer.convert_tokens_to_ids("[CatEmb]")

            item_embs = item_projector(torch.tensor(item_candidates).to(device))  # (K, D)
            cat_embs = category_projector(torch.tensor(category_candidates).to(device))  # (K, D)

            item_idx, cat_idx = 0, 0
            for j, token in enumerate(input_ids):
                if token == item_token_id and item_idx < len(item_embs):
                    inputs_embeds[j] = item_embs[item_idx]
                    item_idx += 1
                elif token == cat_token_id and cat_idx < len(cat_embs):
                    inputs_embeds[j] = cat_embs[cat_idx]
                    cat_idx += 1
        ...
        return {
            'input_ids': torch.vstack(all_input_ids),              # 기존 유지
            'attention_mask': torch.vstack(all_attention_mask),    # 기존 유지
            'labels': torch.vstack(all_labels),                    # 기존 유지
            'inputs_embeds': torch.stack(all_inputs_embeds)        # 새로 추가
        }
...
class LLMWithProjector(nn.Module):
    def __init__(self, base_model, item_projector, category_projector):
        super().__init__()
