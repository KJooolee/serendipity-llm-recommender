...
class Metrics(object):
    ...
    def ndcg(items, **kwargs):
        """
        Binary-relevance NDCG for the given ranked list (already top-k cut by caller).
        items: ranked list/array of item IDs
        kwargs:
          - test_pos: iterable of relevant item IDs
        """
        ...
        # 0-based ranks where the item is relevant
        hits = np.array([i for i, it in enumerate(items) if it in test_pos], dtype=np.int64)
        if hits.size == 0:
            return 0.0

        # DCG: sum 1 / log2(rank+2)
        dcg = np.sum(1.0 / np.log2(hits + 2.0))

        # IDCG: best case with min(#relevant, len(items)) relevants at top
        L = min(len(test_pos), len(items))
        idcg = np.sum(1.0 / np.log2(np.arange(1, L + 1) + 1.0))
        return float(dcg / idcg) if idcg > 0 else 0.0

    @staticmethod
    def mrr(items, **kwargs):
        """
        Mean Reciprocal Rank for this single ranked list.
        Returns 1/(rank of first relevant), else 0.
        """
        items = np.asarray(items)
        test_pos = kwargs['test_pos']
